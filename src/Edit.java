...
public class Edit extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
      ...
      int sl_no = Integer.parseInt(sl);
      String newsl_no=request.getParameter("sl_no");
      ...
      Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/grey_goose","root","Aaditya$Goal");
         String sql = "UPDATE grey_goose.winter_internship SET invoice_currency = ?, cust_payment = ?, WHERE (sl_no =?)";
      PreparedStatement ps = (PreparedStatement) con.prepareStatement(sql);

      
      ps.setLong(1, sl_no);
      ps.setString(2, newinvoice_currency);
      ps.setString(3, newcust_payment);
